...
def calculate_cpr_cdr_time_to_reversion(df, pivots=["time_to_reversion", "product"]):

    # Calculate prepayment
    df.loc[df["prepaid_in_month"], "prepayment"] = (
        df["payment_made"] - df["payment_due"]
    )

    # Calculate amount_default
    df.loc[df["default_in_month"], "amount_default"] = df["current_balance"]
    ...
    aggregated_df = (
        ...
        .agg(
            {
                "amount_default": lambda x: round(x.sum(), 5),
                "prepayment": lambda x: round(x.sum(), 5),
    ...
    # Drop the rows where current_balance is 0
    aggregated_df = aggregated_df[aggregated_df["current_balance"] > 0]

    # Calculate SMM and CPR
    aggregated_df["SMM"] = (
        aggregated_df["prepayment"] / aggregated_df["current_balance"]
    )
    aggregated_df["CPR"] = (1 - (1 - aggregated_df["SMM"]) ** 12) * 100

    # Calculate MDR and CDR
    aggregated_df["MDR"] = (
        aggregated_df["amount_default"] / aggregated_df["current_balance"]
    )
    aggregated_df["CDR"] = (1 - (1 - aggregated_df["MDR"]) ** 12) * 100

    if len(pivots) > 1:
        # Pivot the table for CPR
        pivot_cpr = aggregated_df.pivot(
            index="time_to_reversion", columns=pivots[1:], values="CPR"
        )

        # Pivot the table for CDR
        pivot_cdr = aggregated_df.pivot(
            index="time_to_reversion", columns=pivots[1:], values="CDR"
        )

        # Flatten column names if there are multiple pivot columns
        if len(pivots) > 2:
            pivot_cpr.columns = [
                f"cpr_{'_'.join(map(str, col))}" for col in pivot_cpr.columns.values
            ]
            pivot_cdr.columns = [
                f"cdr_{'_'.join(map(str, col))}" for col in pivot_cdr.columns.values
            ]
        else:
            pivot_cpr.columns = [f"cpr_product_{col}" for col in pivot_cpr.columns]
            pivot_cdr.columns = [f"cdr_product_{col}" for col in pivot_cdr.columns]

        # Combine CPR and CDR pivots
        pivot_table = pd.concat([pivot_cpr, pivot_cdr], axis=1)

        # Sort columns
        pivot_table = pivot_table.reindex(sorted(pivot_table.columns), axis=1)

        # Reset index to make time_to_reversion a column
        pivot_table.reset_index(inplace=True)
    else:
        # If only time_to_reversion is provided, return the aggregated DataFrame
        pivot_table = aggregated_df

    return pivot_table


def set_curve_variables(curve_type):
    """
    Set up curve-specific variables based on the curve type.
    """
    if curve_type == "prepayment":
        ...
        raise ValueError("curve_type must be either 'prepayment' or 'default'")
